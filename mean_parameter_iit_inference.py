...
if __name__ == '__main__':
    ...
    tpm_dim_A = 2**nodes_A
    tpm_dim_B = 2**nodes_B
    population_size_A = 30
    population_size_B = 30
    ...
    for i in range(population_size_A):
        ...
        phis_group_A[i] = compute_phi(cm_A, tpm_A, states_A)
        phis_group_B[i] = compute_phi(cm_B, tpm_B, states_B)
    ...
    stats.ttest_ind(a=phis_group_A, b=phis_group_B)
    ...
    infeasible_A = len(np.where(phis_group_A==0)[0])
    infeasible_B = len(np.where(phis_group_B==0)[0])
    print('Infeasible solutions found for ' + str(nodes_A) + ' nodes: ' + str((infeasible_A/population_size_A)*100) + '%.')
    print('Infeasible solutions found for ' + str(nodes_B) + ' nodes: ' + str((infeasible_B/population_size_B)*100) + '%.')
